...
# 数据库管理类
class Manage:
    def __init__(self, IP='localhost', port=27019, db='testDB'):
    ...
    async def read_file(self, filename: str):
        ...
        for i in range(sec):  # 每1024个点做fft
            contents.iloc[i * 1024:(i + 1) * 1024, 4] = np.log(
                np.abs(fft(np.array(contents.iloc[i * 1024:(i + 1) * 1024, 3]))))
        ...
        init_heatmap = np.array([[float(format(i / 1024, '.2f')),
                                  float(format(y_min + interval * j, '.{}f'.format(significant_digit))), v_min] for i in
                                 range(1024) for j in range(y_num)])
        ...
        for i in range(sec):
            ...
            for j in range(1024):
                k = np.array([[float(format(j / 1024, '.2f')),
                               float(format(second_data.iloc[j, 0], '.{}f'.format(significant_digit))),
                               second_data.iloc[j, 1]]])
