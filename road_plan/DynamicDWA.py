...
# Set random seed
random.seed(0)
global r_odom1, r_odom2, p_odom1, p_odom2
r_odom1 = [0, 0]
r_odom2 = [0, 0]
p_odom1 = [0, 0]
p_odom2 = [0, 0]
...
class Environment:
    ...
    def __init__(self, algorithm, num_obstacles):
        ...
        # self.PLAYFIELDCORNERS = (-300, -200 , 300, 200)   # The region we will fill with obstacles
        self.WIDTH = 1280  # set the width and height of the screen (pixels)
        self.HEIGHT = 960
    ...
    def move_obstacles(self,collection):
        ...
        self.init_obstacle_num = len(latest_doc)
        for key in latest_doc:
            x0, y0, vx, vy = latest_doc[key]['x'],latest_doc[key]['y'],latest_doc[key]['vx'],latest_doc[key]['vy']
            obstacle = [round(x0/75-1.2,2), round(y0/75-1.2,2), round(vx/75-1.2,2), round(vy/75-1.2,2)]
            self.obstacles.append(obstacle)


        for key in latest_doc:
            print(latest_doc[key])
            x0, y0, vx, vy = latest_doc[key]['x'],latest_doc[key]['y'],latest_doc[key]['vx'],latest_doc[key]['vy']
            x0, y0, vx, vy = round(x0/75-1.2,2), round(y0/75-1.2,2), round(vx/75-1.2,2), round(vy/75-1.2,2)
            for n in range(self.pred_pt_num + 3):
                if n > 0:
                    pos_x = x0 + n * self.dt * vx
                    pos_y = y0 + n * self.dt * vy
                    obstacle = [pos_x, pos_y, vx, vy]
                    self.obstacles.append(obstacle)

    def draw_obstacles(self):
        """ Draw circular obstalces on screen """
        global r_odom2, r_odom1, p_odom1, p_odom2

        for (idx, obstacle) in enumerate(self.obstacles):

            if idx < self.init_obstacle_num:
                color = self.lightblue
                pygame.draw.circle(self.screen, color,
                                   (int(self.u0 + self.k * obstacle[0]), int(self.v0 - self.k * obstacle[1])),
                                   int(self.k * self.OBSTACLE_RADIUS), 0)
        ...
        linestart = (self.u0 + self.k * (-1.2), self.v0 - self.k * (-1.2))
        lineend = (self.u0 + self.k * (1.2), self.v0 - self.k * (-1.2))
        pygame.draw.line(self.screen, (0, 255, 255), linestart, lineend, 5)
        linestart = (self.u0 + self.k * (-1.2), self.v0 - self.k * (-1.2))
        lineend = (self.u0 + self.k * (-1.2), self.v0 - self.k * (1.2))
        pygame.draw.line(self.screen, (0, 255, 255), linestart, lineend, 5)
        linestart = (self.u0 + self.k * (-1.2), self.v0 - self.k * (1.2))
    ...
    def draw_goal(self):
        ...
        color = self.red
        goal = self.goal
        pygame.draw.circle(self.screen, color, (int(self.u0 + self.k * goal[0]), int(self.v0 - self.k * goal[1])),
                           int(self.k * self.OBSTACLE_RADIUS), 0)
    ...
    def draw_history_trajectory(self):
        ...
        for pos in self.history_positions:
            pygame.draw.circle(self.screen, color, (int(self.u0 + self.k * pos[0]), int(self.v0 - self.k * pos[1])), 3,
                               0)
    ...
    def check_collsion(self):
        ...
        for obstacle in self.obstacles[0:self.init_obstacle_num]:
    ...
    def run(self):
        ...
        while self.sim_over == False:
            ...
            if self.collision_happened == True:
                self.sim_over = True
                self.sim_times += 1
                print('#{} \t Failure \t [tg:  None] \t  [total collision times:{}]'.format(self.sim_times,
                ...
                print('#{} \t Success \t [tg:  {:.2f}] \t [total collision times:{}]'.format(self.sim_times,
                                                                                             self.time_to_goal,
                                                                                             self.collision_times))
                # time.sleep(5)
                break
                ...
                # Continue simualtion
                r_odom1 = {'position': {'x': self.agent.x, 'y': self.agent.y}}
                self.move_obstacles('human_prediction')  # Move obstacles
...
class NewDWA:
    ...
    def __init__(self, init_pose=(0, 0, 0), config=(0.10, 0, 0, 0)):
        ...
        self.MAX_ACC_LINEAR = 3.2  # ms^(-2) max rate we can change speed of each wheel
        # Angular velocity limits
        self.MAX_VEL_ANGULAR = 6.4
        self.MAX_ACC_ANGULAR = 6.4
        ...
        self.SAFE_DIST = (self.MAX_VEL_LINEAR ** 2 / (2 * self.MAX_ACC_LINEAR)) * 2
        # self.SAFE_DIST = 0.4
        # Weights for predicted trajectory evaluation
        self.FORWARDWEIGHT = 6
        self.OBSTACLEWEIGHT = 66666
    ...
    def predict_position(self, vLpossible, vRpossible, delta_time):
        ...
        # Pure rotation motion
            ...
            # Turn this into  Rect
            Rabs = abs(R)
            ((tlx, tly), (Rx, Ry)) = ((int(self.u0 + self.k * (cx - Rabs)), int(self.v0 - self.k * (cy + Rabs))),
    ...
    def planning(self, goal, obstacles):
        ...
        vLpossiblearray = tuple(
            vLDownBound + i / (sample_num - 1) * (vLUpBound - vLDownBound) for i in range(sample_num))
        ...
        vRpossiblearray = tuple(
            vRDownBound + i / (sample_num - 1) * (vRUpBound - vRDownBound) for i in range(sample_num))
